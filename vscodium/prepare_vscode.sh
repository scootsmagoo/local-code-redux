...
# Local Code npmrc includes electron target/ms_build_id plus pnpm settings (see ../npmrc)
cp ../npmrc .npmrc
...
for i in {1..5}; do # try 5 times
  if [[ "${OS_NAME}" == "osx" ]]; then
    CXX=clang++ pnpm install --frozen-lockfile && break
  else
    pnpm install --frozen-lockfile && break
  fi

  if [[ $i == 5 ]]; then
    echo "pnpm install failed too many times" >&2
    exit 1
  fi
  echo "pnpm install failed $i, trying again..."
